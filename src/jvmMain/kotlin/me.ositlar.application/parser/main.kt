...
val mongo = mongoDatabase.getCollection<FinalSchedule>().apply { drop() }
fun main() {
    ...
    val begin: List<FinalSchedule> = listOf(
        FinalSchedule(
            "Нечетная", "Понедельник", "08:00 - 09:30",
            "лаб.", "теор.осн.ап.-програм.ср.", "доц. СМАЛЕВ А.Н.", "1-329"
        ),
        FinalSchedule(
            "Нечетная", "Пятница", "09:45 - 11:15",
            "лек.", "лек.электротех.и метрология", "доц. ПАШКОВА Н.В.", "1-350"
        ),
        FinalSchedule(
            "Четная", "Понедельник", "08:00 - 09:30",
            "лаб.", "теор.осн.ап.-програм.ср.", "доц. СМАЛЕВ А.Н.", "1-322"
        ),
        FinalSchedule(
            "Четная", "Пятница", "11:30 - 13:00",
            "-", "Физкультура   а.Сз16_", "-"
        )
    )
    begin.forEach {
...
fun controller(file: Document): MutableList<FinalSchedule> {
    ...
    val timeTable = mutableListOf<FinalSchedule>()
                ...
                if (!pe) {
                    ...
                    if (cell[coll].text().contains("доц.")) {
                        ...
                        if (checkCells(typeWeek, days, time, subject))
                            timeTable.add(FinalSchedule(typeWeek, days, time, subjectType, subject, teacher, classroom))
                    }
